...
class EEGformerDecoder(nn.Module):
    def __init__(self, patch_sync, patch_regional) -> None:
        super().__init__()
        self.patch_sync = patch_sync
        self.patch_regional = patch_regional
        self.conv1 = nn.Conv2d(4, 16, kernel_size=3, stride=1, padding=1)  # Output: (8, 16, 11, 121)
        self.conv2 = nn.Conv2d(16, 32, kernel_size=3, stride=1, padding=1)  # Output: (8, 32, 11, 121)
        self.conv3 = nn.Conv2d(32, 64, kernel_size=3, stride=1, padding=1)  # Output: (8, 64, 11, 121)
        self.upsample = nn.ConvTranspose2d(64, 64, kernel_size=4, stride=2, padding=1)  # Output: (8, 64, 22, 242)
        self.upsample2 = nn.ConvTranspose2d(64, 1, kernel_size=4, stride=2, padding=1)  # Output: (8, 1, 44, 484)

        self.fc = nn.Linear(44 * 484, 28 * 28)  # Flattening and connecting to the final output
...
def build_eegformer(
    ...
    eegformer_decoder = EEGformerDecoder(patch_sync, patch_regional)
...
def build_discriminator():
    discriminator = Discriminator(im_chan=1, hidden_dim=64)
    def weights_init(m):
